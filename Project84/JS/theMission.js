...
// empty container / start music / add skip button and credit
...
beginBtn.onclick = function () {
    ...
    /* wait 5 seconds for empty and fade in / create random elements with createOne() every 0.1ms / wait until music drops before clearing everything and fadeToBlack() */
    setTimeout(function () {
...
// stops what its doing / fades to black and empties div / fade in final screen / fade out volume
            ...
            $("#main-text").animate({ opacity: "1" }, 1500, function () {
                $("#main-text").animate({ color: "#69C6DD", marginTop: "5vh" }, 750, function () {
                    $("#stats-box").fadeIn(500, function () {
                        // load in accordian on anything but mobile
                        if ($(window).width() > 500) {
                        ...
                        // load in buttons if on mobile
                        if ($(window).width() < 500) {
                            $(".btn").css("display", "block");
                            $(".btn").animate({ opacity: "1" }, 2000);
                        }
                        // rotate through names displaying them at the bottom of the page
...
// animations for after crazy random elements section
function fadeToBlack() {
    // rotate through different impact text fading in and out before diaplying final screen and fading out volume
    $("#container").remove();
    $("body").animate({ backgroundColor: "#000" }, 1, function () {
                ...
                $("#main-text").animate({ opacity: "0" }, 5000, function () {
                    ...
                    $("#main-text").animate({ opacity: "1" }, 3000, function () {
                        $("#main-text").animate({ opacity: "0" }, 5000, function () {
                            ...
                            $("#main-text").animate({ opacity: "1" }, 3000, function () {
                                //Distort effect / use something premade?
...
//  display the word 'Start' in English, Welsh, Irish and Scottish Gaelic using typeIt to rotate through them with typewriter effect
...
// toggle volume on or off for the slide-show-thing
...
// array of Mens names who have died from suicide in the U.K - found through Wikipedia
...
rotateNames();
//fade names in and out at the bottom of the final page
